...
const SCREENS = {
  ...
  panel: {
    ...
    render: () => {
      ...
      // 방문의향은 절대값으로 못 쓰고 차이는 쓸 수 있다.
      // 재실행 잡음은 **0.7점**이다 (review.py 의 CLEAR_MARGIN 주석, 2026-08-13 실측).
      // 여기 1.8 이라고 적혀 있었는데 근거가 없었다 — 숫자를 두 곳에 따로 적으면
      // 이렇게 갈린다. 기준값 자체는 아래 margin 처럼 서버가 준 것을 쓴다.
      ...
      const cs = copies();
      const rivals = cs.slice(1).map((c) => c.intent ?? 0);
      // 후보가 하나뿐이면 견줄 상대가 없다 — has_clear_winner 도 false 다.
      const gap = rivals.length ? (cs[0]?.intent ?? 0) - Math.max(...rivals) : 0;
            ...
            ${cs.map((c, i) => `
              <div class="card${i === 0 ? ' card--pick' : ''}">
                <div style="display:flex;justify-content:space-between;align-items:baseline">
                  ...
                  ${i === 0 && gap >= margin ? '<span class="data">🏆 손님들이 가장 반응한 문구</span>' : ''}
                </div>
                <strong style="font-size:18px;letter-spacing:-.02em;line-height:1.35">${esc(c.headline)}</strong>
                <span class="muted" style="font-size:14px">${esc(c.sub)}</span>
                ${c.defects.map((d) => `<div class="note note--flag" style="font-size:13px">⚠ ${esc(d)}</div>`).join('')}
