...
window.MOCK = {
  ...
  // 좌표는 **그 주소를 실제로 지오코딩한 값**이다(카카오 REST).
  // 눈대중으로 찍으면 안 된다 — 불광동 56-7 의 좌표를 1.9km 남쪽으로 잘못 두었더니
  // 같은 가게가 좌표로는 '제일시장 전통시장'(객단가 15,933원), 주소로는
  // '연신초등학교 골목상권'(33,963원)으로 잡혔다. 두 배 차이다.
  // 좌표가 있으면 서버가 지오코딩을 건너뛰므로 KAKAO_REST_KEY 없이도 돈다.
  // 번지는 일부러 빼 두었다 — 로그인 전에도 보이는 자리라 실제로 존재하는
  // 번지를 적으면 그 자리의 실제 가게로 오해를 산다 (수호님 보안 점검 S7).
  // 상권 매칭은 아래 좌표가 하므로 표시 주소가 동까지여도 결과는 같다.
  stores: [
    { id: 1, name: '행복한 순대국', industry: '한식음식점', industryId: 'korean_food',
      address: '서울시 은평구 불광동',  lat: 37.627586, lon: 126.927280 },
    { id: 2, name: '홍길동 분식',   industry: '분식·간이음식', industryId: 'snack',
      address: '서울시 마포구 망원동',      lat: 37.557944, lon: 126.906382 },
    { id: 3, name: '마포 커피숍',   industry: '카페·디저트', industryId: 'cafe',
      address: '서울시 마포구 합정동',  lat: 37.544299, lon: 126.912733 },
  ],
  ...
  copies: [
    ...
    // ⚠️ 목업 문구도 AGENTS.md #23 을 지킨다 — 사장님이 말한 것(순대국·8,000원·
    //    퇴근길·따뜻하게)에서만 나온다. 예전에는 '불광동에서 20년'·'가성비 최고'
    //    가 들어 있었는데, 화면에서는 생성된 문구와 구분이 안 돼서 **모델이 지어낸
    //    것으로 읽혔다**(수호님 점검 2번). 없는 연차·지역·가격 평가를 본보기로
    //    보여주면 그게 이 서비스의 기준선이 된다.
    { id: 'c2', headline: '뜨끈한 순대국 한 그릇',       sub: '퇴근길에 들르기 좋은 자리',     intent: 51, defects: ['금액이 문구에 없습니다'] },
    { id: 'c3', headline: '오늘 저녁, 순대국 어떠세요',  sub: '따뜻한 국물로 하루 마무리',     intent: 50, defects: ['금액이 문구에 없습니다'] },
  ],
