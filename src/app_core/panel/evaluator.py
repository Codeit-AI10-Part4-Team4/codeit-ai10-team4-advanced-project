...
**resistance** — 가장 큰 걸림돌을 **딱 하나** 고른다.
  ...
  **기본은 `none` 이다.** 걸림돌은 광고의 흠이 아니라 **네가 발길을 돌리는
  이유**다. 정말 안 가게 만드는 것이 있을 때만 다른 값을 고른다.

  실측(2026-08-11): 동네 평균과 같은 가격(9,500원 vs 9,546원)에도 12명 전원이
  `price` 를 골랐다. 흠을 하나씩 찾아내라고 시키면 없는 흠도 만들어낸다.

  **네가 매긴 방문 의향과 앞뒤가 맞아야 한다.** `intent` 가 네 기준으로 낮지
  않다면 `none` 이다 — 가겠다고 해놓고 걸림돌을 대는 것은 말이 안 된다.
