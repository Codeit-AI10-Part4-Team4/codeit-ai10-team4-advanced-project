...
#: 손님이 이 단계에서 볼 수 없는 것들.
#:
#: 패널이 받는 것은 `CopyCandidate` 뿐이다 — 헤드라인과 서브카피, **글자
#: 두 줄**이 전부다. 사진은 사장님이 문구를 고른 **뒤에** 만들어지므로
#: (`pipeline.render_output`), 평가 시점에는 존재하지 않는다. 사장님이
#: 사진을 미리 올렸더라도 `evaluate()` 로는 넘어오지 않는다.
#:
#: 그런데 `SYSTEM` 이 "광고를 **보고**", "눈에 **걸리는가**" 라고만 하고
#: 그 광고가 무엇인지는 안 말해준다. 모델이 그 빈칸을 사진으로 채운다
#: (실측 2026-08-26, 웹 관통):
#:
#:     40대 여성 [걸리는 것 없음]  국물 사진이 따뜻해 보여서 눈이 갔어요.
#:
#: 이 답은 근거 관문을 그냥 통과했다. `evidence.py` 는 **인용한 숫자**만
#: 대조하는데 이 문장에는 숫자가 없기 때문이다. 없는 것을 근거로 든 답이
#: 그대로 점수가 됐다.
_VISUAL_WORDS: Final = (
    "사진",
    "이미지",
...
#: 봤다고 주장하는 말. 위 낱말과 **같은 절**에 있을 때만 잡는다.
#:
#: "포스터로 붙여두면 눈에 잘 띄겠네요" 는 제안이지 봤다는 주장이 아니다.
#: 낱말만 세면 그것까지 버리게 된다 — `price_contradictions` 에서 두 번
#: 겪은 오탐이라 처음부터 절 단위로 짠다.
_SEEN_WORDS: Final = ("보이", "보여", "봤", "보니", "눈에", "눈이", "비쳐")

#: 절 경계. 어미와 쉼표에서 끊는다 (`eval/panel_metrics.py` 와 같은 방식).
#:
#: `면` 이 들어 있는 이유 — 가정을 뒤 절과 떼기 위해서다. 이게 없으면
#: **"포스터로 붙여두면 눈에 잘 띌 것 같아요"** 가 한 절이 되어 `포스터` 와
#: `눈에` 가 같이 잡힌다. 봤다는 주장이 아니라 제안인데도 버리게 된다
#: (제가 쓴 오탐 검사가 처음 실행에서 바로 잡았다).
#:
#: `화면` 처럼 낱말 안의 `면` 에서도 끊긴다. 그래도 되는 이유는 **끊을수록
#: 절이 좁아져 덜 잡기 때문**이다. 이 함수는 놓치는 쪽으로 기울여야 하므로
#: 과분할은 안전한 방향의 오차다.
_CLAUSE: Final = re.compile(r"[,·]|(?<=지만)|(?<=한데)|(?<=는데)|(?<=아서)|(?<=어서)|(?<=면)")


def _unseen_visuals(comment: str) -> set[str]:
    """보지도 않은 것을 봤다고 한 낱말을 찾는다. 없으면 빈 집합.

    낱말과 "봤다"가 **같은 절**에 있어야 잡는다. 놓치는 쪽으로 기울여 둔다 —
    멀쩡한 답을 버리는 것이 더 나쁘다.
    """
    found: set[str] = set()
    for clause in _CLAUSE.split(comment):
        if not any(s in clause for s in _SEEN_WORDS):
            continue
        found |= {w for w in _VISUAL_WORDS if w in clause}
    return found
