...
def _pool_ratio(p: dict[str, Any], features: dict[str, Any]) -> str:
    """이 연령대가 '있는 만큼 사는가'. **서사에서** 12명을 가르는 유일한 실측값이다.

    `motive` 는 상권 단위라 전원 같고, `price_sens` 는 나이대별 객단가로 갈리지만
    서사에는 일부러 안 넣는다(이유는 `_persona_line`). 이 비율을 주지 않으면
...
def _persona_line(p: dict[str, Any], features: dict[str, Any]) -> str:
    """`motive`·`price_sens` 는 넣지 않는다. 다만 이유가 서로 다르다.

    `motive` 는 상권 단위라 12명이 **전원 같은 값**이다(실측: 상권 300곳 중
    82%가 exploratory). 구별에 기여하지 않으면서 문장만 어지럽힌다 —
    "늘 가던 곳을 다시 찾는 편이라 다양한 가게를 자주 이용한다" 같은 앞뒤가
    안 맞는 문장이 나왔다.

    `price_sens` 는 이제 나이대별 객단가로 **사람마다 갈린다**(2026-08-12).
    그래도 서사에는 안 넣는데, 어차피 평가 프롬프트에 축으로 따로 들어가기
    때문이다(`evaluator._persona_block`). 여기서 또 쓰면 같은 사실이 두 번
    들어가고, 서사는 "손님이 어떤 사람인가"만 맡기로 한 역할 분담이 흐려진다.
    """
    return (
